...
function makeShell(input: {
  ...
  return {
    ...
    ownerUserId: WorkspaceUserId.make("nils"),
    parentThreadId: null,
    workflowRole: null,
    title: "Thread",
...
describe("effectiveSettled", () => {
  ...
  it.each([
    ["failed", { sessionStatus: "error" as const }],
    ["plan-ready", { hasActionableProposedPlan: true }],
  ])("keeps a stale %s thread active until the user settles it", (_label, state) => {
    expect(
      effectiveSettled(makeShell({ activityAt: STALE, ...state }), {
        now: NOW,
        autoSettleAfterDays: 3,
      }),
    ).toBe(false);
    expect(
      effectiveSettled(makeShell({ activityAt: STALE, ...state, settledOverride: "settled" }), {
        now: NOW,
        autoSettleAfterDays: 3,
      }),
    ).toBe(true);
