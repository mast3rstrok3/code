...
lifecycleLayer("CodexAdapterLive lifecycle", (it) => {
  ...
  it.effect("classifies failed Codex turns for workflow recovery", () =>
    Effect.gen(function* () {
      const { adapter, runtime } = yield* startLifecycleRuntime();
      const firstEventFiber = yield* Stream.runHead(adapter.streamEvents).pipe(Effect.forkChild);

      yield* runtime.emit({
        id: asEventId("evt-turn-failed-recovery"),
        kind: "notification",
        provider: ProviderDriverKind.make("codex"),
        createdAt: "2026-01-01T00:00:00.000Z",
      ...
      if (firstEvent._tag === "Some" && firstEvent.value.type === "turn.completed") {
        NodeAssert.deepEqual(firstEvent.value.payload.recovery, {
