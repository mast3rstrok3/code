      ...
      Effect.gen(function* () {
        if (!settings.enabled) {
          return yield* new ProviderAdapterValidationError({
            ...
            ticket: "Enable Antigravity in provider settings before starting a thread.",
          });
        }
        if (
          ...
          return yield* new ProviderAdapterValidationError({
            provider: PROVIDER,
            operation: "startSession",
            ticket: "The Antigravity provider instance does not match the requested session.",
          });
        }
        if (!input.cwd?.trim()) {
          return yield* new ProviderAdapterValidationError({
            provider: PROVIDER,
            operation: "startSession",
            ticket: "The session requires a workspace directory.",
          });
        }
        ...
        if (input.resumeCursor !== undefined && Option.isNone(cursor)) {
          return yield* new ProviderAdapterValidationError({
            provider: PROVIDER,
            operation: "startSession",
            ticket: "The saved Antigravity session is invalid. Start a new thread.",
          });
        }
  ...
  const sendTurn: Adapter["sendTurn"] = Effect.fn("AntigravityAdapter.sendTurn")(function* (input) {
    ...
    if (input.modelSelection && input.modelSelection.instanceId !== options.instanceId) {
      return yield* new ProviderAdapterValidationError({
        provider: PROVIDER,
        operation: "sendTurn",
        ticket: "The selected model belongs to another provider instance.",
      });
    }
    ...
    rollbackThread: (_threadId: ThreadId, _numTurns: number) =>
      Effect.fail(
        new ProviderAdapterValidationError({
          ...
          ticket: "Antigravity does not support conversation rewind. Start a new thread instead.",
        }),
      ),
