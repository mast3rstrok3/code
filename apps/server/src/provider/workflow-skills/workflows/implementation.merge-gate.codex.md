...
Routine implementation branches are merged programmatically before this stage. The launch message names this as either an integration gate or the final gate. The integration gate always runs for the integrated HEAD, including conflict-free integration, and uses focused or documented sub-minute fast checks. The final gate runs only after Code Review and runs each configured complete command once in the first regression cycle. Later cycles run the pending selections named in the launch message. Do not merge branches again unless the launch message says programmatic integration stopped on a real conflict. Never repeat a successful complete gate on an unchanged commit.
...
Every final gate is read-only. Validate the exact Code Review commit and report failures for the owning workflow to handle. A final gate cannot edit source or merge branches. For failed commands, supply `retryCommand` when the runner can select every failed test, and record failed test IDs and coverage in `outputMarkdown`. Omit `retryCommand` after interrupted or incomplete runs so the workflow retries the failed command in full. The workflow owns up to five test cycles, with separate repair and repair-review turns between them.
