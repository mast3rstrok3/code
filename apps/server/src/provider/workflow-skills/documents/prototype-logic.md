...
## T3 prototype variation

Work in the dedicated prototype worktree and branch using the real application and its AppStack. Isolate the logic in a portable module. Wire it into the real application at the route, handler, or service under investigation. Keep the module pure and the application wiring disposable. Expose the full relevant state after every action. When no app entry point exists, use the smallest driver supported by the project task runner in this worktree. Use isolated test state, share the preview URL, and keep the prototype committed on its own branch. These instructions replace the upstream standalone terminal-app shape. Capture the verdict and branch pointer in the durable T3 ticket or Spec.
