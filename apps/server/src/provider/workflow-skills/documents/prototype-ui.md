...
Build variants in a dedicated prototype worktree of the real application and view them through its AppStack. Prefer the existing route with its real layout, data, and authentication. Switch variants with the URL parameter and floating bar described above. Use isolated test state. Capture the verdict and prototype branch pointer in the durable T3 ticket or Spec instead of an external issue. These instructions take precedence over upstream runtime and tracker setup.
