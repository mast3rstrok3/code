...
it.layer(NodeServices.layer)("settled thread decider", (it) => {
  ...
  it.effect("settling a snoozed thread also wakes it", () =>
    Effect.gen(function* () {
      const result = yield* decideOrchestrationCommand({
        ...
        readModel: makeReadModel(null, null, null, [], [], null, {
  ...
  it.effect("rejects settling a workflow root while a descendant session is live", () =>
    Effect.gen(function* () {
      ...
      if (error._tag === "OrchestrationThreadSettleBlockedError") {
        expect(error.threadId).toBe(child.id);
      }
    }),
  );
  ...
  it.effect("repeated settle repairs legacy settled and snoozed state", () =>
    Effect.gen(function* () {
      const result = yield* decideOrchestrationCommand({
        ...
        readModel: makeReadModel("settled", null, null, [], [], null, {
          snoozedUntil: "1970-01-02T09:00:00.000Z",
        }),
      });
      const events = Array.isArray(result) ? result : [result];
  ...
  it.effect("rejects settling a thread with an open approval or user-input request", () =>
    Effect.gen(function* () {
      ...
      expect(openError._tag).toBe("OrchestrationThreadSettleBlockedError");
      ...
      expect(inputError._tag).toBe("OrchestrationThreadSettleBlockedError");
    }),
  );
  ...
  it.effect("clears an open request when its respond failure marks it stale", () =>
    Effect.gen(function* () {
      ...
      expect(stillOpen._tag).toBe("OrchestrationThreadSettleBlockedError");
    }),
  );
  ...
  it.effect("bounds the queued-turn grace window against client clock skew", () =>
    Effect.gen(function* () {
      ...
      expect(queuedError._tag).toBe("OrchestrationThreadSettleBlockedError");
