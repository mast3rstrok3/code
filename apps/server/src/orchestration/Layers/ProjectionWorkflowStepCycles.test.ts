...
const layer = it.layer(
  OrchestrationEngineLive.pipe(
    ...
    Layer.provideMerge(
      ServerConfig.layerTest(process.cwd(), { prefix: "t3-workflow-step-cycles-" }),
    ),
    Layer.provideMerge(NodeServices.layer),
  ),
);

layer("workflow step cycle projection", (it) => {
  it.effect("round-trips a cycle budget through the projection and back into the read model", () =>
    Effect.gen(function* () {
      ...
      const set = yield* snapshotQuery.getCommandReadModel();
      assert.deepEqual(
        set.threads.find((thread) => thread.id === "thread-root")?.workflowStepCycles,
        [
          {
            workflowPromptId: "implementation.browser-app-review.codex",
            stepWorkflowPromptId: "implementation.tdd.codex",
      ...
      yield* engine.dispatch({
        type: "thread.workflow.step-cycles.set",
        commandId: CommandId.make("cmd-clear-cycles"),
        threadId: ThreadId.make("thread-root"),
        workflowPromptId: "implementation.browser-app-review.codex",
        stepWorkflowPromptId: "implementation.tdd.codex",
        maxCycles: null,
      ...
      assert.deepEqual(thread?.workflowStepCycles ?? [], []);
    }),
  );
});
