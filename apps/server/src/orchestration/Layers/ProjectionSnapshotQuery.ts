...
import type { ProjectionThreadDetailQuery } from "../Services/ProjectionSnapshotQuery.ts";
import {
...
import { ProjectionThreadSpec } from "../../persistence/Services/ProjectionThreadSpecs.ts";
...
function mapProjectShellRow(
  ...
  return {
    ...
    previewRecordingMode: row.previewRecordingMode,
...
function mapAppReviewRow(
  ...
  return {
    ...
    ...(row.appReviewScope === null ? {} : { appReviewScope: row.appReviewScope }),
...
function latestSpecByThread(
  rows: ReadonlyArray<Schema.Schema.Type<typeof ProjectionThreadSpecDbRowSchema>>,
): Map<string, OrchestrationPlanningSpec> {
  const latestRows = latestByThread(rows);
  const byThread = new Map<string, OrchestrationPlanningSpec>();
  for (const [threadId, row] of latestRows) {
    byThread.set(threadId, mapSpecRow(row));
  }
  return byThread;
}
...
function buildPlanningWorkflow(input: {
  ...
  if (input.spec === null && input.thread.planningWorkflowStage === null) {
    return null;
  }
  const stage =
...
function planningWorkflowSummary(input: {
  ...
  if (input.thread.planningWorkflowStage === null && input.spec === null) {
    return undefined;
  }
  const stage = input.thread.planningWorkflowStage ?? "grill";
  return {
    stage,
    specId: input.spec?.id ?? null,
    ...(input.spec
      ? {
          specTitle: input.spec.title,
          specSourceThreadId: input.spec.sourceThreadId,
          specWorkflowId: input.spec.workflowId,
          specTicketCount: input.spec.ticketCount,
          specCreatedAt: input.spec.createdAt,
          specUpdatedAt: input.spec.updatedAt,
        }
      : {}),
  };
}
...
function mapThreadShellRow(input: {
  ...
  return {
    ...
    ...(input.thread.workflowStepCycles != null && input.thread.workflowStepCycles.length > 0
      ...
      : {}),
    ...(input.thread.workflowStepReviewParts != null &&
    ...
    branchPullRequest: input.thread.branchPullRequest,
    ...mapThreadPullRequests(input.pullRequests, input.thread.projectId, input.repositoryIdentity),
    ...
    workflowPausedAt: input.thread.workflowPausedAt ?? null,
    unsettledAt: input.thread.unsettledAt,
    snoozedUntil: input.thread.snoozedUntil,
    snoozedAt: input.thread.snoozedAt,
...
const makeProjectionSnapshotQuery = Effect.gen(function* () {
    ...
    execute: () =>
      sql`
        SELECT
          ...
          preview_recording_mode AS "previewRecordingMode",
  ...
  const listThreadSpecRows = SqlSchema.findAll({
    Request: Schema.Void,
    Result: ProjectionThreadSpecDbRowSchema,
    execute: () =>
      sql`
        SELECT
          spec_id AS "specId",
        ...
        ORDER BY thread_id ASC, created_at ASC, spec_id ASC
      `,
  });

  const listThreadPlanningTicketRows = SqlSchema.findAll({
    Request: Schema.Void,
    Result: ProjectionThreadPlanningTicketDbRowSchema,
    execute: () =>
      sql`
        SELECT
        ...
        ORDER BY thread_id ASC, ordinal ASC, created_at ASC, ticket_id ASC
      `,
  });
  ...
  const listThreadSpecRowsByThread = SqlSchema.findAll({
    Request: ThreadIdLookupInput,
    Result: ProjectionThreadSpecDbRowSchema,
    execute: ({ threadId }) =>
      sql`
  ...
  const listThreadPlanningTicketRowsByThread = SqlSchema.findAll({
    Request: ThreadIdLookupInput,
    Result: ProjectionThreadPlanningTicketDbRowSchema,
    execute: ({ threadId }) =>
      sql`
...
pending_approval_requests AS (
      ...
      .withTransaction(
        Effect.all([
          ...
          listThreadSpecRows(undefined).pipe(
            Effect.mapError(
              toPersistenceSqlOrDecodeError(
                "ProjectionSnapshotQuery.getSnapshot:listThreadSpecs:query",
                "ProjectionSnapshotQuery.getSnapshot:listThreadSpecs:decodeRows",
              ),
            ),
          ),
          listThreadPlanningTicketRows(undefined).pipe(
            Effect.mapError(
              toPersistenceSqlOrDecodeError(
                "ProjectionSnapshotQuery.getSnapshot:listThreadPlanningTickets:query",
                "ProjectionSnapshotQuery.getSnapshot:listThreadPlanningTickets:decodeRows",
              ),
            ),
          ),
          ...
          ([
            ...
            Effect.gen(function* () {
              ...
              const threads: ReadonlyArray<OrchestrationThread> = threadRows.map((row) => ({
                ...
                workflowPausedAt: row.workflowPausedAt ?? null,
      ...
      .withTransaction(
        Effect.all([
          ...
          listThreadSpecRows(undefined).pipe(
            Effect.mapError(
              toPersistenceSqlOrDecodeError(
                "ProjectionSnapshotQuery.getCommandReadModel:listThreadSpecs:query",
                "ProjectionSnapshotQuery.getCommandReadModel:listThreadSpecs:decodeRows",
              ),
            ),
          ),
          listThreadPlanningTicketRows(undefined).pipe(
            Effect.mapError(
              toPersistenceSqlOrDecodeError(
                "ProjectionSnapshotQuery.getCommandReadModel:listThreadPlanningTickets:query",
                "ProjectionSnapshotQuery.getCommandReadModel:listThreadPlanningTickets:decodeRows",
              ),
            ),
          ),
          listThreadPlanningReviewCycleRows(undefined).pipe(
          ...
          ([
            ...
            Effect.gen(function* () {
              ...
              for (let index = 0; index < planningTicketRows.length; index += 1) {
                const row = planningTicketRows[index];
                if (!row) {
                  continue;
                }
                updatedAt = maxIso(updatedAt, row.updatedAt);
      ...
      .withTransaction(
        Effect.all([
          ...
          listThreadSpecRows(undefined).pipe(
            Effect.mapError(
              toPersistenceSqlOrDecodeError(
                "ProjectionSnapshotQuery.getShellSnapshot:listThreadSpecs:query",
                "ProjectionSnapshotQuery.getShellSnapshot:listThreadSpecs:decodeRows",
              ),
            ),
          ),
          listImplementationRunRows(undefined).pipe(
        ...
        Effect.flatMap(
          ([
            ...
            Effect.gen(function* () {
              ...
              const visibleSpecRows = specRows.filter((row) => visibleThreadIds.has(row.threadId));
              const specByThread = latestSpecByThread(visibleSpecRows);
              ...
              for (const row of visibleSpecRows) {
                updatedAt = maxIso(updatedAt, row.updatedAt);
              }
      ...
      .withTransaction(
        Effect.all([
          ...
          listThreadSpecRows(undefined).pipe(
            Effect.mapError(
              toPersistenceSqlOrDecodeError(
                "ProjectionSnapshotQuery.getArchivedShellSnapshot:listThreadSpecs:query",
                "ProjectionSnapshotQuery.getArchivedShellSnapshot:listThreadSpecs:decodeRows",
              ),
            ),
          ),
          listImplementationRunRows(undefined).pipe(
            ...
            : repositoryIdentityResolver.resolve(option.value.workspaceRoot).pipe(
                Effect.map((repositoryIdentity) =>
                  Option.some({
                    ...
                    previewRecordingMode: option.value.previewRecordingMode,
  ...
  const getThreadShellById: ProjectionSnapshotQueryShape["getThreadShellById"] = (
    ...
    Effect.gen(function* () {
      const [threadRow, latestTurnRow, sessionRow, specRows, pullRequestRows] = yield* Effect.all([
        ...
        listThreadSpecRowsByThread({ threadId }).pipe(
          Effect.mapError(
            toPersistenceSqlOrDecodeError(
              "ProjectionSnapshotQuery.getThreadShellById:listThreadSpecs:query",
              "ProjectionSnapshotQuery.getThreadShellById:listThreadSpecs:decodeRows",
            ),
          ),
        ),
    ...
    Effect.gen(function* () {
      ...
      const [
        ...
        listWorkflowSubagentBatchRowsByThread({ threadId }).pipe(
          Effect.mapError(
            toPersistenceSqlOrDecodeError(
              "ProjectionSnapshotQuery.getThreadDetailById:listWorkflowSubagentBatches:query",
              "ProjectionSnapshotQuery.getThreadDetailById:listWorkflowSubagentBatches:decodeRows",
            ),
          ),
        ),
        listWorkflowSubagentBatchChildRowsByThread({ threadId }).pipe(
          Effect.mapError(
            toPersistenceSqlOrDecodeError(
              "ProjectionSnapshotQuery.getThreadDetailById:listWorkflowSubagentBatchChildren:query",
              "ProjectionSnapshotQuery.getThreadDetailById:listWorkflowSubagentBatchChildren:decodeRows",
            ),
          ),
        ),
        listThreadSpecRowsByThread({ threadId }).pipe(
          Effect.mapError(
            toPersistenceSqlOrDecodeError(
              "ProjectionSnapshotQuery.getThreadDetailById:listThreadSpecs:query",
              "ProjectionSnapshotQuery.getThreadDetailById:listThreadSpecs:decodeRows",
            ),
          ),
        ),
        listThreadPlanningTicketRowsByThread({ threadId }).pipe(
          Effect.mapError(
            toPersistenceSqlOrDecodeError(
              "ProjectionSnapshotQuery.getThreadDetailById:listThreadPlanningTickets:query",
              "ProjectionSnapshotQuery.getThreadDetailById:listThreadPlanningTickets:decodeRows",
            ),
          ),
        ),
      ...
      const artifactThreadRow =
        canonicalSpec === undefined || canonicalSpec.sourceThreadId === threadRow.value.threadId
          ...
          : yield* getActiveThreadRowById({ threadId: canonicalSpec.sourceThreadId }).pipe(
              Effect.mapError(
                toPersistenceSqlOrDecodeError(
                  "ProjectionSnapshotQuery.getThreadDetailById:getArtifactThread:query",
                  "ProjectionSnapshotQuery.getThreadDetailById:getArtifactThread:decodeRow",
                ),
              ),
            );
      const thread = {
        ...
        threadRow.value.workflowStepCycles.length > 0
          ...
          : {}),
        ...(threadRow.value.workflowStepReviewParts != null &&
        ...
        workflowPausedAt: threadRow.value.workflowPausedAt ?? null,
  ...
  const getThreadDetailById: ProjectionSnapshotQueryShape["getThreadDetailById"] = (
    ...
    getThreadDetailByIdBounded(threadId, undefined, { mode: "raw", ...(query ? { query } : {}) });
