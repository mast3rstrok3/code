...
import {
  ...
  workflowParkedRateLimits,
  workflowQueuedStagesWithoutOwners,
  workflowStageLaunchLatency,
...
type CodeReviewDirective = {
  ...
  /** Set when a review-and-fix cycle landed fixes; names the commit it produced. */
  readonly commitSha?: string;
  readonly validations: ReadonlyArray<OrchestrationImplementationValidationResult>;
...
export function findAwaitingNestedAppReview(
  runs: OrchestrationReadModel["appReviewWorkflowRuns"],
  runId: string,
...
function reopenTicketForRerun(input: {
  ...
  const queuedGeneration = (state: OrchestrationImplementationTicketState) =>
    state.stageExecutions.findLast(
      (execution) =>
        execution.state === "queued" &&
        execution.target.kind === "ticket" &&
        execution.target.ticketId === input.ticketId &&
        execution.target.stage === input.stage,
    )?.generation;
    ...
    executions.map((execution) =>
      ...
      execution.target.stage === input.stage
        ? {
            ...
            leaseRenewedAt: input.updatedAt,
            leaseExpiresAt: DateTime.formatIso(
              DateTime.add(DateTime.makeUnsafe(Date.parse(input.updatedAt)), { minutes: 5 }),
            ),
            lastProgressAt: input.updatedAt,
  ...
  const ticketStates = input.run.ticketStates.map((state) => {
    ...
    if (input.stage === "code-review") {
      const generation = queuedGeneration(state) ?? state.codeReviewGeneration + 1;
      return {
        ...cleared,
        ...
        stageExecutions: claimExecution(state.stageExecutions, generation),
      };
    }
    ...
    if (input.stage === "app-review") {
      const generation = queuedGeneration(state) ?? state.appReviewGeneration + 1;
      return {
        ...withoutAppReview,
    ...
    // The worker starts another turn in its durable thread. Its reported
    // result stops standing for this ticket, while the thread, worktree,
    // branch, partial changes, and conversation history survive.
    const generation = queuedGeneration(state) ?? state.implementationGeneration + 1;
    return {
      ...withoutAppReview,
...
function clearRunStageForRerun(input: {
  ...
  const queued = input.run.stageExecutions.findLast(
    (execution) =>
      execution.state === "queued" &&
      execution.target.kind === "run" &&
      execution.target.stage === input.stage,
  );
  const claimedExecutions = input.run.stageExecutions.map((execution) =>
    execution.executionId === queued?.executionId
      ? {
          ...execution,
          state: "starting" as const,
          claimedAt: input.updatedAt,
  ...
  switch (input.stage) {
    ...
    case "code-review":
      return {
        ...
        finalCodeReviewGeneration: queued?.generation ?? input.run.finalCodeReviewGeneration + 1,
  ...
  const continuationLines =
    input.inheritsPartialChanges || input.continuationMarkdown !== null
      ? [
          "",
          "Continuation state:",
          input.inheritsPartialChanges
            ? "- This ticket worktree contains tracked or untracked changes from an earlier turn."
            : "- Continue in this ticket's original durable thread and worktree. Preserve its useful commits and conversation history.",
...
function buildMergeGatePrompt(input: {
    ...
    input.kind === "final"
      ? [
          input.run.codeReviewExhaustedAt === null
            ? "Code Review accepted the current HEAD. Do not change code or merge branches in this gate; validate that exact reviewed commit."
            : "Code Review exhausted its cycle budget on the current committed HEAD. Do not change code or merge branches in this gate; validate that exact reviewed commit before publication.",
        ]
  ...
  const continuationInstructions =
    input.run.retryableFailure?.stage === "merge-gate"
      ? [
          "",
          "Continue this Merge Gate in its existing durable thread. Resolve the prior failure before reporting:",
          input.run.retryableFailure.detail,
        ]
      : [];
  return [
...
function changeRequestReviewNote(run: OrchestrationImplementationRun): string {
  ...
  if (run.codeReviewExhaustedAt !== null) {
    lines.push(
      `- ⚠️ Final Code Review exhausted its cycle budget. ${run.codeReviewExhaustionReason ?? "The latest findings remain unresolved."}`,
    );
    if (run.latestCodeReviewReportMarkdown !== null) {
      lines.push(
        `- Latest final Code Review report: ${run.latestCodeReviewReportMarkdown.slice(0, 1_000)}`,
      );
    }
  }
  return lines.join("\n");
...
const make = Effect.gen(function* () {
  ...
  const updateRun = Effect.fn("ImplementationWorkflowReactor.updateRun")(function* (input: {
    ...
    readonly expectedStageExecutionTransition?: {
      readonly target: WorkflowStageExecution["target"];
      readonly generation: number;
      readonly executionId: string;
      readonly priorState: WorkflowStageExecution["state"];
      readonly priorLeaseExpiresAt: string | null;
    };
  ...
  const createWorker = Effect.fn("ImplementationWorkflowReactor.createWorker")(function* (input: {
    ...
    readonly createdAt: string;
    readonly claimedBeforeStart: boolean;
  }) {
    ...
    const readModel = yield* projectionSnapshotQuery.getCommandReadModel();
    const durableWorkerThread =
      findThread(readModel, workerThreadId) ??
      Option.getOrNull(yield* projectionSnapshotQuery.getThreadDetailById(workerThreadId));
    const isContinuation = input.claimedBeforeStart;
    if (
      isContinuation &&
      (durableWorkerThread === null || durableWorkerThread.deletedAt !== null)
    ) {
      return yield* new GitCommandError({
        operation: "ImplementationWorkflowReactor.createWorker",
        command: "continue durable implementation thread",
        cwd: plannedWorker.worktreePath,
        detail: `Ticket '${input.ticketId}' cannot continue because its durable Implementation thread '${workerThreadId}' is missing or deleted. Human attention is required.`,
      });
    }
      ...
      for (let pass = 0; pass <= input.run.ticketStates.length; pass += 1) {
        ...
        const workerThreadIds = new Map(
          yield* Effect.forEach(
            ...
            (ticketId) => {
              const durableThreadId = workingRun.ticketStates.find(
                (state) => state.ticketId === ticketId,
              )?.workerThreadId;
              return durableThreadId === null || durableThreadId === undefined
                ? serverThreadId("implementation-worker").pipe(
                    Effect.map((threadId) => [ticketId, threadId] as const),
                  )
                : Effect.succeed([ticketId, durableThreadId] as const);
            },
          ),
        );
        const claimedBeforeStart = new Set(
          workingRun.ticketStates
            .filter(
              (state) => readyTicketIds.includes(state.ticketId) && state.workerThreadId !== null,
            )
            .map((state) => state.ticketId),
        );
          ...
          (ticketId) =>
            Effect.result(
              createWorker({
                ...
                createdAt: input.createdAt,
                claimedBeforeStart: claimedBeforeStart.has(ticketId),
              }),
  ...
  const startTicketCodeReview = Effect.fn("ImplementationWorkflowReactor.startTicketCodeReview")(
    function* (input: {
      ...
      const reviewerThreadId =
        state.codeReviewThreadId ?? (yield* serverThreadId("implementation-code-reviewer"));
      const existingReviewer = findThread(readModel, reviewerThreadId);
      if (existingReviewer?.deletedAt !== null && existingReviewer !== null) {
        yield* blockRun({
          sourceThreadId: input.sourceThreadId,
          run: input.run,
          ticketId: state.ticketId,
      ...
      if (
        existingReviewer?.session?.status === "starting" ||
        existingReviewer?.session?.status === "running"
      ) {
        return;
      }
      ...
      if (state.codeReviewPassCount >= cycleBudget) {
        ...
        yield* finishTicketReviewChain({
          ...
          codeReviewOutcome: "findings",
          warningMarkdown: [
            state.warningMarkdown ?? "",
            `Ticket Code Review exhausted ${cycleBudget} cycle${cycleBudget === 1 ? "" : "s"} with unresolved findings.`,
          ]
            .filter(Boolean)
            .join("\n\n"),
          createdAt: input.createdAt,
      ...
      if (existingReviewer === null) {
        yield* orchestrationEngine.dispatch({
          ...
          workflowContext,
          title: `Code review ${ticket?.title ?? input.ticketId} · Cycle ${state.codeReviewPassCount + 1} of ${cycleBudget}`,
          modelSelection: yield* modelForStep({
            workflowPromptId: WORKFLOW_PROMPT_IDS.implementationCodeReviewCodex,
  ...
  const startTicketAppReviewUnlocked = Effect.fn(
    ...
    if (
      isTicketStageSkipped(currentRun.skips, input.ticketId, "app-review") ||
    ...
    const configuredParts = resolveLayeredAppReviewStepParts({
    ...
    if (effectiveScope === null) {
      yield* startTicketCodeReview({
        ...
        run: currentRun,
        ticketId: input.ticketId,
        warningMarkdown: `Ticket App Review skipped because its effective E2E and browser scope is empty. Configured parts: ${describeAppReviewParts(configuredParts)}.`,
  ...
  const startBrowserReview = Effect.fn("ImplementationWorkflowReactor.startBrowserReview")(
    function* (input: {
      ...
      const continueWithoutBrowserReview = Effect.fn(
        ...
        const skippedRun: OrchestrationImplementationRun = {
          ...run,
          latestAppReviewWorkflowOutcome: "skipped",
          updatedAt: input.createdAt,
        };
        if (
          skippedRun.integrationHeadSha !== null &&
          skippedRun.codeReviewedHeadSha === skippedRun.integrationHeadSha
        ) {
          yield* startMergeGate({
            sourceThreadId: input.sourceThreadId,
            run: skippedRun,
            integration: {
              baseTicketId: null,
              baseRefName: skippedRun.orchestratorBranch,
              mergedTicketIds: [],
              conflictedTicketId: null,
              conflictedRefName: null,
              conflictedFiles: [],
  ...
  const startCodeReview = Effect.fn("ImplementationWorkflowReactor.startCodeReview")(
    function* (input: {
      ...
      const cycleBudget = finalPass
        ? yield* cyclesForStep({
            key: { workflowPromptId: WORKFLOW_PROMPT_IDS.implementationCodeReviewCodex },
            orchestratorThread,
          })
        : 1;
      if (finalPass && input.run.finalCodeReviewPassCount >= cycleBudget) {
        yield* fileChangeRequest({
          sourceThreadId: input.sourceThreadId,
          run: {
            ...input.run,
            status: "publishing-change-request",
            codeReviewExhaustedAt: input.run.codeReviewExhaustedAt ?? input.createdAt,
            codeReviewExhaustionReason:
              input.run.codeReviewExhaustionReason ??
              `Final Code Review exhausted ${cycleBudget} cycles with unresolved findings.`,
          },
          createdAt: input.createdAt,
        });
        return;
      ...
      const artifactMarkdown =
        input.run.artifactSource === "proposed-plan" && artifactSourceThread !== null
          ...
          : undefined;
      const reviewerThreadId =
        input.run.activeCodeReviewThreadId ??
        (yield* serverThreadId("implementation-code-reviewer"));
      const existingReviewer = findThread(readModel, reviewerThreadId);
      if (existingReviewer?.deletedAt !== null && existingReviewer !== null) {
        yield* blockRun({
          sourceThreadId: input.sourceThreadId,
          run: input.run,
          reasonMarkdown: `Code Review cannot continue because Cycle ${input.run.finalCodeReviewPassCount + 1} of ${cycleBudget}'s durable thread '${reviewerThreadId}' was deleted.`,
          updatedAt: input.createdAt,
        ...
        message: {
          ...
          text: appendWorkflowSkillCommandSection(
            buildCodeReviewPrompt({
              run: reviewingRun,
              cycleNumber: finalPass ? input.run.finalCodeReviewPassCount + 1 : 1,
              cycleBudget,
              ...(artifactMarkdown === undefined ? {} : { artifactMarkdown }),
  ...
  const startChangeRequestBabysitter = Effect.fn(
    ...
    const babysitterThreadId =
      input.run.activeChangeRequestBabysitterThreadId ??
      (yield* serverThreadId("implementation-change-request-babysitter"));
    const existingBabysitter = findThread(readModel, babysitterThreadId);
    if (existingBabysitter?.deletedAt !== null && existingBabysitter !== null) {
      yield* blockRun({
        sourceThreadId: input.sourceThreadId,
        run: input.run,
        reasonMarkdown: `Pull-request babysitting cannot recover because its durable thread '${babysitterThreadId}' was deleted.`,
        updatedAt: input.createdAt,
        humanBlocked: true,
      });
      return;
    ...
    if (
      existingBabysitter?.session?.status === "starting" ||
      existingBabysitter?.session?.status === "running"
    ) {
      return;
    }
  ...
  const fileChangeRequestUnlocked = Effect.fn(
    ...
    if (expectedHeadSha === null) {
      yield* blockRun({
        ...
        run: input.run,
        retryableStage: "code-review",
        reasonMarkdown: "Cannot publish a change request before Code Review accepts an exact HEAD.",
        updatedAt: input.createdAt,
      });
      return;
    }
        ...
        const failedRun = failImplementationTickets(
          {
            ...run,
            ticketStates: run.ticketStates.map((state) =>
              state.workerThreadId === threadId ? { ...state, workerResult: directive } : state,
            ),
  ...
  const handleCodeReviewResult = Effect.fn("ImplementationWorkflowReactor.handleCodeReviewResult")(
    function* (
      ...
      if (run !== null && directive.ticketId !== undefined) {
        ...
        if (state.codeReviewPassCount >= cycleBudget) return;
        const [head, status] = yield* Effect.all([
          gitWorkflow.resolveCommit({ cwd: state.worktreePath, ref: "HEAD" }),
          gitWorkflow.localStatus({ cwd: state.worktreePath }),
        ]);
        ...
        const completedReviewRun: OrchestrationImplementationRun = {
          ...
          retryableFailure:
            ...
            run.retryableFailure.ticketId === state.ticketId
              ? null
              : run.retryableFailure,
          ticketStates: run.ticketStates.map((candidate) =>
            candidate.ticketId === directive.ticketId
        ...
        if (directive.status === "findings" || directive.status === "blocked") {
          ...
          const findingsWarning =
            nextPassCount >= cycleBudget
              ? `Ticket Code Review exhausted ${cycleBudget} cycle${cycleBudget === 1 ? "" : "s"} with an unresolved ${directive.status} result. Latest report:\n\n${directive.reportMarkdown}`
              : null;
          const nextCycleRun: OrchestrationImplementationRun = {
            ...completedReviewRun,
            ticketStates: completedReviewRun.ticketStates.map((candidate) =>
              candidate.ticketId === state.ticketId
                ? {
                    ...candidate,
                    codeReviewThreadId: null,
                    codeReviewGeneration: candidate.codeReviewGeneration + 1,
                    codeReviewLaunchCount: 0,
                    warningMarkdown: [state.warningMarkdown ?? "", findingsWarning ?? ""]
                      .filter(Boolean)
                      .join("\n\n"),
                    workerResult:
          ...
          yield* finishTicketReviewChain({
            ...
            usableBranch: true,
            warningMarkdown:
              nextCycleRun.ticketStates.find((candidate) => candidate.ticketId === state.ticketId)
                ?.warningMarkdown ?? findingsWarning,
            createdAt: updatedAt,
          });
          return;
        }
      ...
      const cycleBudget = finalPass
        ...
        : 1;
      if (finalPass && run.finalCodeReviewPassCount >= cycleBudget) return;
      const [head, status] = yield* Effect.all([
        gitWorkflow.resolveCommit({ cwd: run.orchestratorWorktreePath, ref: "HEAD" }),
        gitWorkflow.localStatus({ cwd: run.orchestratorWorktreePath }),
      ]);
      ...
      const requiredCompleteCommands = completeValidationCommandsForFiles(run, changedFiles);
      if (finalPass) {
        if (directive.status === "clean" && run.activeCodeReviewHeadSha !== head.commitSha) {
          yield* blockRun({
            sourceThreadId,
            run,
            retryableStage: "code-review",
            reasonMarkdown: `Final Code Review reported "clean" but HEAD moved from the reviewed commit '${run.activeCodeReviewHeadSha ?? "unknown"}' to '${head.commitSha}'. Report "findings" with a commitSha when the review lands changes.`,
            updatedAt,
            humanBlocked: true,
          });
          return;
        }
        if (directive.status === "findings" && directive.commitSha === undefined) {
          yield* blockRun({
            sourceThreadId,
            run,
            retryableStage: "code-review",
            reasonMarkdown:
              "Final Code Review reported findings without naming the commit that fixes them. Fix the findings, commit them, and report commitSha.",
            updatedAt,
            humanBlocked: true,
          });
          return;
        ...
        if (reportedHead !== null && reportedHead.commitSha !== head.commitSha) {
          yield* blockRun({
            sourceThreadId,
            run,
            retryableStage: "code-review",
            reasonMarkdown: `Final Code Review reported commit '${reportedHead.commitSha}', but the orchestrator branch HEAD is '${head.commitSha}'.`,
            updatedAt,
            humanBlocked: true,
          });
          return;
        ...
        const completedCycleCount = run.finalCodeReviewPassCount + 1;
        const atCeiling = completedCycleCount >= cycleBudget;
        const completeValidationPassed = completeValidationsPassedExactlyOnce({
          requiredCommands: requiredCompleteCommands,
          validations: directive.validations,
        });
        const focusedValidationPassed = focusedRepairValidationsPassed({
          finalCommands: requiredCompleteCommands,
          validations: directive.validations,
        });
        const validationSummaryResult = validationSummary(
          directive.validations,
        ...
        const reviewedRun: OrchestrationImplementationRun = {
          ...
          finalCodeReviewPassCount: completedCycleCount,
          finalValidation: completeValidationRequired
            ? {
                ...validationSummaryResult,
                status: completeValidationPassed ? "passed" : "failed",
                outputMarkdown: completeValidationPassed
        ...
        if (directive.status === "clean" && completeValidationPassed) {
          yield* fileChangeRequest({
            sourceThreadId,
            run: { ...reviewedRun, status: "publishing-change-request" },
            createdAt: updatedAt,
          });
          return;
        }
        ...
        if (!atCeiling) {
          const nextCycleRun: OrchestrationImplementationRun = {
            ...reviewedRun,
            status: "code-reviewing",
            finalCodeReviewGeneration: reviewedRun.finalCodeReviewGeneration + 1,
            finalCodeReviewLaunchCount: 0,
            codeReviewExhaustedAt: null,
            codeReviewExhaustionReason: null,
            latestCodeReviewReportMarkdown: [
              directive.reportMarkdown,
              ...(validationPassed ? [] : [validationFailureMarkdown]),
            ].join("\n\n"),
            updatedAt,
          };
        ...
        const exhaustionReason = [
          `Final Code Review exhausted ${cycleBudget} cycle${cycleBudget === 1 ? "" : "s"} with a ${directive.status} result.`,
          ...(validationPassed ? [] : [validationFailureMarkdown]),
          `Latest report:\n\n${directive.reportMarkdown}`,
        ].join("\n\n");
        const exhaustedRun: OrchestrationImplementationRun = {
        ...
        yield* appendActivity({
          threadId: run.orchestratorThreadId,
          tone: "info",
          kind: "implementation-code-review-exhausted",
          summary: `Final Code Review exhausted after ${cycleBudget} cycles`,
          payload: {
            runId: run.id,
            cycles: cycleBudget,
            reasonMarkdown: exhaustionReason,
      ...
      if (!validationsValid) {
        yield* blockRun({
          ...
          retryableStage: "code-review",
          reasonMarkdown:
            "Code Review did not report passing focused validation, or ran a complete validation command before final validation.",
          updatedAt,
        });
        return;
      }
      ...
      if (directive.status === "clean") {
        ...
        const reviewedRun: OrchestrationImplementationRun = {
          ...
          activeCodeReviewThreadId: null,
          latestCodeReviewReportMarkdown: directive.reportMarkdown,
          finalValidation: finalPass ? null : run.finalValidation,
          finalValidationResults: finalPass ? [] : run.finalValidationResults,
          validatedHeadSha: finalPass ? null : run.validatedHeadSha,
      ...
      if (finalPass) {
        const exhausted = reviewedRun.finalCodeReviewPassCount >= cycleBudget;
        const cycleRun: OrchestrationImplementationRun = {
          ...reviewedRun,
          integrationHeadSha: head.commitSha,
          finalCodeReviewGeneration: exhausted
            ? reviewedRun.finalCodeReviewGeneration
            : reviewedRun.finalCodeReviewGeneration + 1,
          finalCodeReviewLaunchCount: exhausted ? reviewedRun.finalCodeReviewLaunchCount : 0,
          codeReviewExhaustedAt: exhausted ? updatedAt : null,
          codeReviewExhaustionReason: exhausted
            ? [
                `Final Code Review exhausted ${cycleBudget} cycle${cycleBudget === 1 ? "" : "s"} with an unresolved ${directive.status} result.`,
                "Focused Code Review validation passed. Final validation still must pass on this HEAD.",
                `Latest report:\n\n${directive.reportMarkdown}`,
              ].join("\n\n")
            : null,
          finalValidation: null,
        ...
        yield* appendActivity({
          ...
          payload: {
            runId: run.id,
            cycles: cycleBudget,
            reasonMarkdown: cycleRun.codeReviewExhaustionReason,
          },
          createdAt: updatedAt,
        });
        yield* startMergeGate({
  ...
  const resumeTicketAppReview = Effect.fn("ImplementationWorkflowReactor.resumeTicketAppReview")(
    function* (input: {
      ...
      const readModel = yield* projectionSnapshotQuery.getCommandReadModel();
      if (
        findAwaitingNestedAppReview(readModel.appReviewWorkflowRuns, input.nestedRun.id) === null
      ) {
        return;
      }
      const orchestratorThread = findThread(readModel, input.run.orchestratorThreadId);
      ...
      const nestedRun = findAwaitingNestedAppReview(
        refreshedReadModel.appReviewWorkflowRuns,
        input.nestedRun.id,
      );
      const currentRun = findRunById(refreshedReadModel, input.run.id);
      const currentTicket = currentRun?.ticketStates.find(
        (candidate) => candidate.ticketId === input.ticketId,
      );
      if (
        nestedRun === null ||
        currentTicket === undefined ||
        !ticketAwaitsAppReviewRun(currentTicket, nestedRun.id)
      ) {
        return;
      }
      yield* orchestrationEngine.dispatch({
   ...
   */
  const runAcceptedRerun = Effect.fn("ImplementationWorkflowReactor.runAcceptedRerun")(
    function* (input: {
      readonly readModel: OrchestrationReadModel;
      readonly run: OrchestrationImplementationRun;
      ...
      const { readModel, run, target, createdAt } = input;
      const sourceThreadId = findRunSourceThreadId({ readModel, run });
      if (sourceThreadId === null) return;
      const queuedExecution =
        target.kind === "run"
          ? run.stageExecutions.findLast(
              (execution) =>
                execution.target.kind === "run" &&
                execution.target.stage === target.stage &&
                execution.state === "queued",
            )
          : run.ticketStates
              .find((ticket) => ticket.ticketId === target.ticketId)
              ?.stageExecutions.findLast(
                (execution) =>
                  execution.target.kind === "ticket" &&
                  execution.target.stage === target.stage &&
                  execution.state === "queued",
              );
      const expectedStageExecutionTransition =
        queuedExecution === undefined
          ? undefined
          : {
              target: queuedExecution.target,
              generation: queuedExecution.generation,
              executionId: queuedExecution.executionId,
              priorState: queuedExecution.state,
              priorLeaseExpiresAt: queuedExecution.leaseExpiresAt,
            };
      const resumesCanceledFinalCodeReview =
        run.status === "canceled" && target.kind === "run" && target.stage === "code-review";

      // A pause can land after acceptance. The queued execution remains durable
      // and workflow reconciliation claims it after the pause clears.
      if (
      ...
      if (target.kind === "run") {
        ...
        yield* updateRun({
          ...
          run: rerunRun,
          createdAt,
          ...(expectedStageExecutionTransition === undefined
            ? {}
      ...
      if (outcome !== "passed") {
        ...
        if (outcome === "exhausted") {
          yield* startTicketCodeReview({
            ...
            run: reviewedTicketRun,
            ticketId,
            warningMarkdown: warningMarkdown ?? "Ticket App Review exhausted its cycle budget.",
            createdAt: input.updatedAt,
          });
          return;
        }
    ...
    if (!passed) {
      ...
      yield* updateRun({
        ...
        run: {
          ...run,
          status: "needs-human-attention",
          // A retry adds another turn to the one durable babysitter thread.
          activeChangeRequestBabysitterThreadId: run.activeChangeRequestBabysitterThreadId,
          retryableFailure: {
    ...
    for (const run of readModel.implementationRuns) {
      ...
      if (failedTicketWarnings.size > 0) {
        ...
        const hasStrandedDependent = run.ticketStates.some(
          (state, index) =>
            state.status !== "failed" && reconciledRun.ticketStates[index]?.status === "failed",
        );
        if (hasStrandedDependent) {
          const sourceThreadId = findRunSourceThreadId({ readModel, run });
          if (sourceThreadId !== null) {
            yield* recoverRunStage(
              run.id,
              "failed-ticket-dependencies",
              updateRun({ sourceThreadId, run: reconciledRun, createdAt }),
            );
          }
          continue;
        }
  ...
  const reconcileCanonicalStageExecutions = Effect.fn(
    "ImplementationWorkflowReactor.reconcileCanonicalStageExecutions",
  )(function* () {
    let readModel = yield* projectionSnapshotQuery.getCommandReadModel();
    const createdAt = DateTime.formatIso(yield* DateTime.now);
    const actions = reconcileWorkflowState(readModel, createdAt);
    const executions = readModel.implementationRuns.flatMap((run) => [
      ...run.stageExecutions,
      ...run.ticketStates.flatMap((ticket) => ticket.stageExecutions),
    ]);
    yield* Metric.update(
      workflowQueuedStagesWithoutOwners,
      executions.filter((execution) => execution.state === "queued").length,
    );
    yield* Metric.update(
      workflowParkedRateLimits,
      executions.filter(
        (execution) =>
          execution.state === "retry-wait" && execution.failure?.category === "provider-rate-limit",
      ).length,
    );

    for (const action of actions) {
      if (action.type === "claim-queued-stage") {
        const queuedAtMs = Date.parse(action.execution.queuedAt);
        const claimedAtMs = Date.parse(createdAt);
        if (!Number.isNaN(queuedAtMs) && !Number.isNaN(claimedAtMs)) {
          yield* Metric.update(
            Metric.withAttributes(
              workflowStageLaunchLatency,
              metricAttributes({ targetKind: action.execution.target.kind }),
            ),
            Duration.millis(Math.max(0, claimedAtMs - queuedAtMs)),
          );
        }
        if (!action.execution.executionId.startsWith("workflow-execution-")) continue;
        const executionTarget = action.execution.target;
        if (executionTarget.kind === "app-review-phase") continue;
        const target: OrchestrationImplementationRerunTarget | null =
          executionTarget.kind === "ticket"
            ? executionTarget.stage === "validation"
              ? null
              : {
                  kind: "ticket",
                  ticketId: executionTarget.ticketId,
                  stage: executionTarget.stage,
                }
            : executionTarget.stage === "integration" ||
                executionTarget.stage === "merge-gate" ||
                executionTarget.stage === "app-review" ||
                executionTarget.stage === "code-review"
              ? { kind: "run", stage: executionTarget.stage }
              : null;
        if (target === null) continue;
        const run = findRunById(readModel, executionTarget.runId);
        if (run === null) continue;
        yield* runAcceptedRerun({ readModel, run, target, createdAt });
        readModel = yield* projectionSnapshotQuery.getCommandReadModel();
        continue;
      }

      if (
        action.type === "expire-lease" ||
        action.type === "leave-retry-wait" ||
        action.type === "revoke-stale-execution"
      ) {
        if (action.type === "expire-lease") {
          yield* Metric.update(
            Metric.withAttributes(
              workflowStaleLeasesTotal,
              metricAttributes({ targetKind: action.execution.target.kind }),
        ...
        const ticket =
          executionTarget.kind === "ticket"
            ? run.ticketStates.find((candidate) => candidate.ticketId === executionTarget.ticketId)
            : null;
        const nestedReview =
          executionTarget.stage === "app-review"
            ? (readModel.appReviewWorkflowRuns ?? []).find(
                (candidate) =>
                  candidate.caller.type === "implementation" &&
                  candidate.caller.implementationRunId === run.id &&
                  (executionTarget.kind === "run"
                    ? candidate.caller.ticketId === undefined
                    : candidate.caller.ticketId === executionTarget.ticketId),
              )
            : undefined;
        const ownerThreadId =
          executionTarget.kind === "ticket"
            ? executionTarget.stage === "implementation"
              ? ticket?.workerThreadId
              : executionTarget.stage === "code-review"
                ? ticket?.codeReviewThreadId
                : nestedReview?.activeThreadId
            : executionTarget.stage === "app-review"
              ? (nestedReview?.activeThreadId ?? run.activeAppReviewThreadId)
              : executionTarget.stage === "code-review"
                ? run.activeCodeReviewThreadId
                : executionTarget.stage === "babysitting"
                  ? run.activeChangeRequestBabysitterThreadId
                  : run.activeValidatorThreadId;
        const owner =
          ownerThreadId == null
            ? undefined
            : readModel.threads.find((thread) => thread.id === ownerThreadId);
        const ownerIsLive =
          owner?.session?.status === "starting" ||
          owner?.session?.status === "running" ||
          owner?.session?.activeTurnId != null;
        if (action.type === "expire-lease" && owner === undefined) {
          yield* Metric.update(
            Metric.withAttributes(
              workflowMissingChildExecutionsTotal,
              metricAttributes({ targetKind: action.execution.target.kind }),
        ...
        const nextExecution: WorkflowStageExecution =
          action.type === "revoke-stale-execution"
            ? {
                ...action.execution,
                state: "skipped",
            ...
            : action.type === "expire-lease" && ownerIsLive
              ? {
                  ...action.execution,
                  state: "running",
                  ...
                  failure:
                    action.type === "expire-lease"
                      ? {
                          ...
                          detail: "The stage lease expired without a confirmed live owner.",
                          failedAt: createdAt,
                          nextAction: "continue-stage",
                        }
                      : action.execution.failure,
                  recovery:
                    action.execution.recovery === null
                      ? null
                      : { ...action.execution.recovery, retryAt: null },
                  updatedAt: createdAt,
                };
        const replace = (execution: WorkflowStageExecution) =>
          execution.executionId === action.execution.executionId ? nextExecution : execution;
        const nextRun =
          executionTarget.kind === "ticket"
            ? {
                ...run,
                ticketStates: run.ticketStates.map((candidate) =>
                  candidate.ticketId === executionTarget.ticketId
                    ? { ...candidate, stageExecutions: candidate.stageExecutions.map(replace) }
                    : candidate,
                ),
                updatedAt: createdAt,
              }
            : {
                ...run,
                stageExecutions: run.stageExecutions.map(replace),
                updatedAt: createdAt,
              };
        yield* updateRun({
          sourceThreadId,
          run: nextRun,
          expectedStageExecutionTransition: {
            target: action.execution.target,
            generation: action.execution.generation,
            executionId: action.execution.executionId,
            priorState: action.execution.state,
            priorLeaseExpiresAt: action.execution.leaseExpiresAt,
          },
          createdAt,
        });
        readModel = yield* projectionSnapshotQuery.getCommandReadModel();
      ...
      if (
        action.type === "derive-dependency-block" ||
        action.type === "derive-dependency-eligibility"
      ) {
        const run = findRunById(readModel, action.runId);
        if (run === null) continue;
        const sourceThreadId = findRunSourceThreadId({ readModel, run });
        if (sourceThreadId === null) continue;
        ...
        const ticketStates = run.ticketStates.map((ticket) => {
          ...
          const dependencyExecution: WorkflowStageExecution = {
            target: {
              kind: "ticket",
              runId: run.id,
            ...
            failure: {
              ...
              detail: `Waiting for dependencies: ${action.dependencyTicketIds.join(", ")}.`,
              failedAt: createdAt,
        ...
        const nextRun = { ...run, ticketStates, updatedAt: createdAt };
        yield* updateRun({ sourceThreadId, run: nextRun, createdAt });
        if (action.type === "derive-dependency-eligibility") {
          yield* startReadyWorkers({ sourceThreadId, run: nextRun, createdAt });
        }
        readModel = yield* projectionSnapshotQuery.getCommandReadModel();
        continue;
      }

      if (action.type === "report-watchdog") {
        const run = findRunById(readModel, action.runId);
        if (run === null) continue;
        yield* Metric.update(
          Metric.withAttributes(
            workflowWatchdogFindingsTotal,
            metricAttributes({ nextAction: action.nextAction }),
          ),
          1,
        );
        yield* orchestrationEngine.dispatch({
          type: "thread.activity.append",
          commandId: CommandId.make(action.commandId),
          threadId: run.orchestratorThreadId,
          activity: {
            id: EventId.make(`${action.commandId}:activity`),
            tone: "error",
            kind: "workflow-watchdog-finding",
            summary: `Workflow needs action: ${action.nextAction}`,
            payload: {
              runId: run.id,
  ...
  const reconcileStartup = Effect.fn("ImplementationWorkflowReactor.reconcileStartup")(
    function* () {
      yield* reconcileCanonicalStageExecutions().pipe(
        Effect.catchCause((cause) =>
          Effect.logWarning("implementation workflow canonical reconciliation failed", {
            cause: Cause.pretty(cause),
          }),
        ),
      );
  ...
  return {
    ...
    drain: worker.drain,
    flush: worker.flush,
    reconcileStartup: () => reconcileStartup().pipe(Effect.orDie),
