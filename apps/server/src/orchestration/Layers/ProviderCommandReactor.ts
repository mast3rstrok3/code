...
const make = Effect.gen(function* () {
  ...
  const addWorktreeRuntimeContext = Effect.fn("ProviderCommandReactor.addWorktreeRuntimeContext")(
    function* (input: { readonly thread: OrchestrationThreadShell; readonly messageText: string }) {
      ...
      const activities = yield* projectionThreadActivities.listByThreadId({
      ...
      const context = buildWorktreeRuntimeContext({
        ...
        setupFailureDetail: (() => {
          const latestSetupOutcome = activities
            .toReversed()
            .find(
              (activity) =>
        ...
        stackFailureDetail: (() => {
          const latestStackOutcome = activities
            .toReversed()
  ...
  type QueuedTurnStart = Pick<TurnStartRequestedEvent, "type" | "payload"> & {
  ...
  const formatFailureDetail = (cause: Cause.Cause<unknown>): string => {
    ...
    if (isProviderAdapterValidationError(failReason?.error)) {
      return failReason.error.ticket;
    }
  ...
  const processTurnStartRequested = Effect.fn("processTurnStartRequested")(function* (
    ...
    if (!resumed && (yield* hasHandledTurnStartRecently(key))) {
      return;
    }
        ...
        Effect.flatMap(() =>
          appendProviderFailureActivity({
            ...
            recovery: classifyProviderFailure({
              error: cause.reasons.find(Cause.isFailReason)?.error,
              message: detail,
    ...
    const acknowledgeTurnStart = Effect.fnUntraced(function* (result: ProviderTurnStartResult) {
      const current = yield* resolveThreadShell(result.threadId);
