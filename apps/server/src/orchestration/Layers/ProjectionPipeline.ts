...
const makeOrchestrationProjectionPipeline = Effect.fn("makeOrchestrationProjectionPipeline")(
  function* () {
        ...
        case "project.meta-updated": {
          ...
          yield* projectionProjectRepository.upsert({
            ...
            ...(event.payload.defaultThreadEnvMode !== undefined
              ...
              : {}),
            ...(event.payload.previewRecordingMode !== undefined
              ? { previewRecordingMode: event.payload.previewRecordingMode }
              : {}),
    ...
    const applyThreadsProjection: ProjectorDefinition["apply"] = Effect.fn(
      ...
      switch (event.type) {
        ...
        case "thread.workflow-step-review-parts-set": {
          ...
          // Keyed by step and sub-step together — see the projector.
          const otherReviewParts = (existingRow.value.workflowStepReviewParts ?? []).filter(
            (entry) =>
              entry.workflowPromptId !== event.payload.workflowPromptId ||
              entry.stepWorkflowPromptId !== event.payload.stepWorkflowPromptId,
          );
        ...
        case "thread.planning-spec-bundle-loaded": {
          ...
          yield* projectionThreadRepository.upsert({
            ...existingRow.value,
            planningWorkflowStage: event.payload.bundle === undefined ? null : "completed",
    ...
    const applyThreadPlanningReviewCyclesProjection: ProjectorDefinition["apply"] = Effect.fn(
    ...
    const applyImplementationRunsProjection: ProjectorDefinition["apply"] = Effect.fn(
      "applyImplementationRunsProjection",
    )(function* (event, _attachmentSideEffects) {
      switch (event.type) {
