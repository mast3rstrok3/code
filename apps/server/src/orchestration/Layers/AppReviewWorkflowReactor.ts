...
export const APP_REVIEW_FIXER_IMPLEMENTATION_ONLY_INSTRUCTION =
  "This is an implementation-only phase. Do not call preview_* or app_review_* tools, collect browser evidence, or update the review verdict. The workflow owns verification after it receives your app-review-fix-result directive, so unavailable review tools must not block this repair.";
...
export function appReviewFixValidationsPassed(
  ...
  return (
    focused.length > 0 &&
    focused.every((validation) => validation.status === "passed") &&
      ...
      : focusedFailures.length === 0
        ? [
            hasPostRepairVerification(input.validations)
              ...
              : "Report a passing verification after the latest reproduction run.",
          ]
        : focusedFailures),
...
export function claimAppReviewValidationRepair(input: {
  ...
  if (
    ...
    return null;
  const currentValidations = currentWorkflowValidations(input.result.validations);
...
export function isSupersededAppReviewPhaseThread(
  ...
  return run.cycles.some(
    (cycle) =>
      ...
      (!cycle.e2eExecution && cycle.reviewerThreadId === threadId) ||
...
export function buildE2eReviewPrompt(input: {
  ...
  return appendWorkflowSkillCommandSection(
    [
      ...
      "If provisioning or a controller action is required, record blocked checks with blockerKind external-prerequisite and the missing prerequisite, its owner, and the action needed to resume. Workflow-service or MCP failures outside the reviewed worktree belong to that service's owner, not product repair findings. Use another authorized artifact read tool when available; block only the checks whose required evidence remains unavailable. Keep these external blockers in notes and nextSteps; actionable findings describe product defects or repairable acceptance coverage gaps. Preserve any actual failed command results. Resume acceptance after the prerequisite changes.",
...
export function buildAppReviewFixPrompt(input: {
  ...
  return appendWorkflowSkillCommandSection(
    [
      ...
      ...(input.cycle.e2eExecution
        ? [
            "The workflow runs E2E commands programmatically in the next cycle of this same run after accepting your repair. Use small unit or integration checks for development. Return retryCommands as [{command, retryCommand}] selecting every failed test in each failed suite; command must exactly match a failed original suite below, with at most one mapping per suite. Use supported selectors, retain all failures in the last selection, and explain coverage in notesMarkdown. Omit a mapping when the runner cannot narrow it; the workflow reruns that suite's last selection. Passing suites stay closed. Do not launch or watch the full E2E suites in this turn.",
            ...completedAppReviewTests(input.cycle.e2eExecution)
              .filter((result) => result.status === "failed")
              .map(
                (result) =>
                  `Failed suite: ${result.command}
Last selection: ${result.executedCommand}
...
export function buildAppReviewFixResultContinuationPrompt(input: {
  ...
  return appendWorkflowSkillCommandSection(
    [
      ...
      ...(input.cycle.e2eExecution
        ? [
            "Preserve the repair's retryCommands in your result. Each mapping must name a failed original suite exactly once and select every failure from its last execution. An omitted mapping reruns the previous selection. The server executes those tests in the next cycle.",
          ]
        : []),
...
const make = Effect.gen(function* () {
  ...
  /** Load E2E configuration without hiding invalid files behind missing-command errors. */
      ...
      Effect.tapError((error) =>
        Effect.gen(function* () {
          yield* failRun({
            ...
            retryable: true,
            detailMarkdown: `${error.message}\nCorrect the project configuration and resume this App Review. Completed repairs remain on the ticket branch.`,
            occurredAt: DateTime.formatIso(yield* DateTime.now),
          });
        }),
      ),
    );
    return resolveAppReviewE2eCommands(Option.getOrUndefined(projectFile));
  });
  ...
  const interruptActivePhaseTurn = Effect.fn("AppReviewWorkflowReactor.interruptActivePhaseTurn")(
    function* (run: AppReviewWorkflowRun, occurredAt: string) {
      const process = testProcesses.get(run.id);
      if (process) {
  ...
  const ensureProgrammaticE2e = Effect.fn("AppReviewWorkflowReactor.ensureProgrammaticE2e")(
    function* (run: AppReviewWorkflowRun, cycle: AppReviewWorkflowCycle, preflightPassed = false) {
      const execution = cycle.e2eExecution;
  ...
  const ensureE2eLaunch = Effect.fn("AppReviewWorkflowReactor.ensureE2eLaunch")(function* (
    ...
    if (cycle.e2eExecution) {
      yield* ensureProgrammaticE2e(run, cycle, preflightPassed);
      return;
    }
  ...
  const startReview = Effect.fn("AppReviewWorkflowReactor.startReview")(function* (
    ...
    if (includesE2e && e2eCommands.length === 0) {
      yield* failRun({
        run,
        reason: "automation-unavailable",
        detailMarkdown: isTicketAppReview(run)
          ? "This ticket has no structured appReviewCommands. Add its focused suite commands to the ticket plan before restarting App Review."
          : "Configure all application E2E suites in t3.json e2eCommands before starting App Review.",
        occurredAt,
      });
      return;
    }
    ...
    if (retryFailure !== null) {
      yield* failRun({
        ...
        reason: "fixer-failed",
        retryable: false,
        detailMarkdown: retryFailure,
        occurredAt,
      });
      return;
    }
    const cycleNumber = run.cyclesUsed + 1;
    const e2eThreadId = null;
    const cycle: AppReviewWorkflowCycle = {
      ...
      reviewerThreadId: includesE2e
        ? run.controllerThreadId
        : yield* serverThreadId("app-review-reviewer"),
    ...
    const reviewingRun: AppReviewWorkflowRun = {
      ...
      activeThreadId: includesE2e ? null : cycle.reviewerThreadId,
      updatedAt: occurredAt,
    };
  ...
  const reviewRecordForCycle = (
    ...
    (cycle.e2eExecution ? programmaticReviewRecord(run, cycle) : null) ??
  ...
  const failCycle = Effect.fn("AppReviewWorkflowReactor.failCycle")(function* (input: {
    ...
    switch (phase) {
      ...
      case "planning": {
        const controller = yield* resolveThread(run.controllerThreadId);
        const review =
          controller === undefined ? null : reviewRecordForCycle(run, controller, cycle);
      ...
      message: {
        ...
        text: appendWorkflowSkillCommandSection(
          [
            ...
            "",
            cycle.e2eExecution
              ? "The server ran the E2E commands programmatically. The brief and command results below are the input for this planning thread. Write repair tickets here; a separate fixer thread implements them, then the server retests the affected selections in the next cycle."
              : "The review that produced these findings ran in a separate thread; the brief and the complete actionable findings below are the whole input.",
            "Do not edit files, browse the app, or ask questions. Apply the To Tickets vertical-slice discipline to every actionable finding.",
            ...
            "Plan all independent code, coverage, and test-setup repairs even when external prerequisites remain. Each repair ticket must belong to the reviewed product and be implementable in its authorized worktree. Classify workflow-service and MCP defects outside that worktree as external prerequisites, even when the reviewer listed them as actionable findings. Keep those defects, real credentials, billing, approvals, and operator actions in the plan summary with their owner and recovery action; do not create product repair tickets for them or treat them as completed.",
  ...
  const reconcileE2e = Effect.fn("AppReviewWorkflowReactor.reconcileE2e")(function* (
    ...
    const cycle = run.cycles.at(-1);
    if (run.activePhase === "e2e" && cycle?.e2eExecution) {
      const execution = cycle.e2eExecution;
      if (execution.commands.length === 0) {
        yield* failRun({
          ...
          reason: "automation-unavailable",
          retryable: false,
          detailMarkdown: "The E2E cycle has no selected commands and cannot verify a repair.",
          occurredAt,
        });
        return;
      }
      const results = completedAppReviewTests(execution);
      if (results.length < execution.commands.length) {
        yield* ensureProgrammaticE2e(run, cycle);
        return;
      }
      testProcesses.delete(run.id);
      const target = yield* resolveTarget(run.targetThreadId);
      if (target === null) return;
      const stable = yield* assertStableRevision(run, target.cwd, occurredAt);
      if (stable === null) return;
      const failed = results.filter((result) => result.status === "failed");
      const verdict = failed.length ? ("failed" as const) : ("passed" as const);
      const completed = {
        ...stable,
        cycles: stable.cycles.map((entry) =>
          entry.cycleNumber === cycle.cycleNumber ? { ...entry, e2eVerdict: verdict } : entry,
        ),
        updatedAt: occurredAt,
      };
...
${result.outputMarkdown}`,
        ...
        .join("\n\n");
      const review = programmaticReviewRecord(run, cycle);
      if (!failed.length) yield* finishPassed(completed, review, occurredAt);
      else if (run.cyclesUsed >= run.cycleBudget) {
        yield* finishExhausted(
          {
            ...completed,
            cycles: completed.cycles.map((entry) =>
              entry.cycleNumber === cycle.cycleNumber
                ? {
                    ...entry,
                    status: "review-failed" as const,
                    reviewVerdict: "failed" as const,
                    completedAt: occurredAt,
                    actionableFindingsMarkdown: findings,
                  }
                : entry,
            ),
          },
      ...
      cycles: stableRun.cycles.map((entry) =>
        entry.cycleNumber === cycle.cycleNumber
          ? {
              ...
              e2eVerdict,
              deferredValidationCommands:
                isImplementationAppReview(run) && !cycle.e2eExecution
  ...
  const parseFixResult = (
    ...
    for (const activity of thread.activities.toReversed()) {
      ...
      return {
        ...
        ...(Array.isArray(payload["retryCommands"])
          ? {
              retryCommands: payload["retryCommands"] as NonNullable<
                AppReviewWorkflowFixResult["retryCommands"]
              >,
            }
          : {}),
        validations,
  ...
  const reconcileFixer = Effect.fn("AppReviewWorkflowReactor.reconcileFixer")(function* (
    ...
    const caller = run.caller;
    const retryFailure = appReviewRetryCommandsFailure(cycle, result);
    if (retryFailure !== null) {
      yield* failRun({
        run: reportedRun,
        reason: "fixer-failed",
    ...
    const completeValidationCommands =
      caller.type === "implementation"
        ...
        : [];
    const projectValidationCommands = yield* e2eCommandsForCwd(target.cwd, run);
  ...
  const rerunPhase = Effect.fn("AppReviewWorkflowReactor.rerunPhase")(function* (
    ...
    if (phase === "e2e" || phase === "review") {
      ...
      yield* startReview(reopened, occurredAt, true);
      return;
    }
  ...
  const processEvent = Effect.fn("AppReviewWorkflowReactor.processEvent")(function* (
    ...
    if (event.type === "thread.workflow-paused" || event.type === "thread.workflow-resumed") {
      yield* reconcileRuns();
      return;
    }

    if (event.type === "thread.app-review-workflow-cancel-requested") {
      const process = testProcesses.get(event.payload.run.id);
      if (process) {
        yield* Fiber.interrupt(process.fiber);
        testProcesses.delete(event.payload.run.id);
      }
  ...
  const recoverFailedImplementationPhase = Effect.fn(
    ...
    const planningReview =
      claim.phase === "planning" && controller !== undefined
        ? reviewRecordForCycle(input.run, controller, cycle)
        : null;
    if (
  ...
  const reconcileRuns = Effect.fn("AppReviewWorkflowReactor.reconcileRuns")(function* () {
    ...
    for (const [runId, process] of testProcesses) {
      const current = readModel.appReviewWorkflowRuns?.find((entry) => entry.id === runId);
      if (
        !current ||
        current.status !== "running" ||
  ...
  const recordTestResult = Effect.fn("AppReviewWorkflowReactor.recordTestResult")(function* (
    item: Extract<WorkItem, { kind: "test-result" }>,
  ) {
    const readModel = yield* projectionSnapshotQuery.getCommandReadModel();
    const run = readModel.appReviewWorkflowRuns?.find((entry) => entry.id === item.runId);
    const cycle = run?.cycles.at(-1);
    if (
      !run ||
      run.status !== "running" ||
          ...
          Effect.catchCause((cause) =>
            Effect.gen(function* () {
              ...
              if (process?.id === item.executionId) {
                yield* Fiber.interrupt(process.fiber);
                testProcesses.delete(item.runId);
              }
              const readModel = yield* projectionSnapshotQuery.getCommandReadModel();
              const run = readModel.appReviewWorkflowRuns?.find((entry) => entry.id === item.runId);
              if (
                run?.activePhase === "e2e" &&
                run.cycles.at(-1)?.e2eExecution?.id === item.executionId
              ) {
                yield* failRun({
                  run,
                  reason: "automation-unavailable",
                  retryable: false,
                  detailMarkdown:
                    "The workflow could not save a test result. Completed results remain recorded. Resume after restoring storage.",
                  occurredAt: item.result.completedAt,
                });
              }
            }).pipe(
              Effect.catchCause((failure) =>
                Effect.logError("E2E result recovery failed", Cause.pretty(failure)),
              ),
            ),
          ),
        )
  ...
  const start: AppReviewWorkflowReactorShape["start"] = Effect.fn("start")(function* () {
    ...
    yield* Effect.forkScoped(
      Stream.runForEach(domainEvents, (event) => {
        ...
        if (
          event.type !== "thread.workflow-paused" &&
...
export const programmaticReviewRecord = (
  ...
  const results = cycle.e2eExecution ? completedAppReviewTests(cycle.e2eExecution) : [];
  const commands = [
    ...new Set(
      run.cycles.flatMap((entry) =>
        (entry.e2eExecution?.commands ?? []).map((selection) => selection.command),
      ),
    ),
  ];
  const failed =
    results.length === 0 ||
    results.length !== cycle.e2eExecution?.commands.length ||
    results.some((result) => result.status === "failed");
  const verdict = failed ? ("failed" as const) : ("passed" as const);
  return {
    id: cycle.e2eReviewId ?? cycle.reviewId,
    sourceThreadId: run.controllerThreadId,
    reviewThreadId: run.controllerThreadId,
    appReviewScope: "e2e",
    sourceTurnId: null,
    status: verdict,
    evidence: EMPTY_APP_REVIEW_EVIDENCE,
    document: {
      verdict,
      summary: results.map((result) => `${result.command}: ${result.status}`).join("\n"),
      checks: results.map((result) => ({
        id: `e2e-${commands.indexOf(result.command) + 1}`,
        label: result.command,
        status: result.status,
        notes: `Executed: ${result.executedCommand}\n${result.outputMarkdown}`,
        evidenceIds: [],
      })),
      findings: [],
      questions: [],
    ...
    updatedAt: results.reduce(
      (latest, result) => (result.completedAt > latest ? result.completedAt : latest),
      cycle.startedAt,
    ),
  };
};
