...
describe("OrchestrationEngine", () => {
  ...
  effectIt.effect("persists a rejected receipt when settlement is blocked", () =>
    Effect.gen(function* () {
      ...
      const error = yield* engine
        ...
        .pipe(Effect.flip);
      const detail = `thread ${threadId} has an active session and cannot be settled`;
      const message = `Orchestration command invariant failed (thread.settle): ${detail}`;
  ...
  effectIt.effect("processes interactive commands before queued background commands", () =>
    Effect.gen(function* () {
      ...
      const gatedStore: OrchestrationEventStoreShape = {
        ...
        readAggregateRange: (range) =>
          Stream.fromIterable(
            events.filter(
              (event) =>
                event.aggregateKind === range.aggregateKind &&
                event.aggregateId === range.aggregateId &&
                event.sequence > range.fromSequenceExclusive &&
                event.sequence <= range.toSequenceInclusive,
            ),
          ),
        getAggregateReplayStats: () => Effect.die("unused"),
        hasEventAfter: () => Effect.succeed(false),
      };
      const ServerConfigLayer = ServerConfig.layerTest(process.cwd(), {
