...
describe("orchestration projector", () => {
  effectIt.effect("applies thread.created events", () =>
    Effect.gen(function* () {
  ...
  effectIt.effect("fails when event payload cannot be decoded by runtime schema", () =>
    Effect.gen(function* () {
      const now = "2026-01-01T00:00:00.000Z";
      const model = createEmptyReadModel(now);
  ...
  effectIt.effect("applies thread.archived and thread.unarchived events", () =>
    Effect.gen(function* () {
      const now = "2026-01-01T00:00:00.000Z";
  ...
  effectIt.effect("keeps projector forward-compatible for unhandled event types", () =>
    Effect.gen(function* () {
      const now = "2026-01-01T00:00:00.000Z";
      const model = createEmptyReadModel(now);
  ...
  effectIt.effect("tracks latest turn id from session lifecycle events", () =>
    Effect.gen(function* () {
  ...
  effectIt.effect("updates canonical thread runtime mode from thread.runtime-mode-set", () =>
    Effect.gen(function* () {
      const createdAt = "2026-02-23T08:00:00.000Z";
  ...
  effectIt.effect("merges and clears workflow step model pins", () =>
    Effect.gen(function* () {
      ...
      const afterFirstPin = yield* projectEvent(
        afterCreate,
        pinStep(2, "implementation.tdd.codex", claudeSelection),
      );
      const afterSecondPin = yield* projectEvent(
        afterFirstPin,
        pinStep(3, "implementation.code-review.codex", codexSelection),
      );
      ...
      const afterRepin = yield* projectEvent(
        afterSecondPin,
        pinStep(4, "implementation.tdd.codex", codexSelection),
      );
      ...
      const afterClear = yield* projectEvent(
        afterRepin,
        pinStep(5, "implementation.tdd.codex", null),
      );
  ...
  effectIt.effect("marks assistant messages completed with non-streaming updates", () =>
    Effect.gen(function* () {
