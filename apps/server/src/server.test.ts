...
it.layer(NodeServices.layer)("server router seam", (it) => {
  ...
  it.effect("returns 404 for missing static assets instead of the SPA document", () =>
    Effect.gen(function* () {
      const fileSystem = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const staticDir = yield* fileSystem.makeTempDirectoryScoped({ prefix: "t3-router-static-" });
      yield* fileSystem.writeFileString(path.join(staticDir, "index.html"), "<html>ready</html>");

      yield* buildAppUnderTest({ config: { staticDir } });

      const response = yield* HttpClient.get("/assets/AppReviewPanel-stale.js");
      assert.equal(response.status, 404);
      assert.notInclude(yield* response.text, "<html>");
    }).pipe(Effect.provide(NodeHttpServer.layerTest)),
  );
    ...
    () =>
      Effect.gen(function* () {
        ...
        yield* buildAppUnderTest({
          layers: {
            keybindings: {
              loadConfigState: Effect.succeed({ keybindings: [], tickets: [] }),
              streamChanges: Stream.empty,
            },
            providerRegistry: {
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const runForThread = vi.fn(
          (
            ...
            Effect.succeed({
              ...
              completion: Deferred.await(setupCompleted).pipe(
        ...
        expect(runForThread.mock.calls[0]?.[0]).toEqual({
  ...
  it.effect("does not misattribute setup activity dispatch failures as setup launch failures", () =>
    Effect.gen(function* () {
      ...
      const runForThread = vi.fn(
        (
          ...
          Effect.succeed({
            ...
            // Awaited, so the completion is recorded before the turn starts.
            async: false,
          }),
      );
      ...
      assert.deepEqual(
        ...
        ["worktree-setup", "setup-script.requested", "setup-script.completed", "worktree-setup"],
      );
      assertTrue(
