        ...
        Effect.gen(function* () {
          ...
          const runSetupProgram = () =>
            Effect.gen(function* () {
              ...
              if (setupResult.status === "no-script" || !setupResult.completion) return true;
              ...
              if (completion.exitCode === 0) {
                yield* track(worktreeSetupTracker.stageStatus(threadId, "setup-script", "done"));
                yield* recordSetupScriptCompleted({
                  worktreePath,
                  scriptId: setupResult.scriptId,
                  scriptName: setupResult.scriptName,
                  terminalId: setupResult.terminalId,
          ...
          const bootstrapProgram = Effect.gen(function* () {
            ...
            const finalResult = yield* Effect.uninterruptible(dispatchFromClient(finalCommand));
            yield* track(
              worktreeSetupTracker
