  ...
  WsRpcGroup.toLayer(
    Effect.gen(function* () {
      ...
      const canReplayPersistedRange = Effect.fnUntraced(function* (
        ...
        if (replayGap < 0 || replayGap > maxGap) return false;
        if (projectionSnapshotQuery.getEventReplayStats === undefined) return true;
        ...
        if (stats.payloadBytes <= ORCHESTRATION_REPLAY_PAYLOAD_BUDGET_BYTES) return true;
      ...
      const toShellEvent = (event: OrchestrationEvent) => {
        switch (event.type) {
          ...
          default:
            ...
            return { type, aggregateKind, aggregateId, sequence };
        }
      };
      ...
      const toShellStreamEvent = (
        ...
        switch (event.type) {
          ...
          case "thread.archived":
            return Effect.succeed(removeThreadIfVisible(ThreadId.make(event.aggregateId)));
        ...
        Effect.gen(function* () {
          ...
          const bootstrapRuntimeMode =
            command.type === "thread.turn.start"
              ? command.runtimeMode
              : (bootstrap?.createThread?.runtimeMode ?? DEFAULT_RUNTIME_MODE);
          ...
          // Starts the setup script. `pending` is the fiber that waits for an
          // `async` script to exit and records the outcome on the card; it is
          // null when nothing is left to await. `ready` says whether the
          // dependencies the script installs can be relied on, which gates the
          // workflow App Stack.
          const runSetupProgram = () =>
            Effect.gen(function* () {
              ...
              const setupResult = yield* projectSetupScriptRunner
                ...
                .pipe(
                  Effect.matchEffect({
                    ...
                    onSuccess: (
                      setupResult,
                    ): Effect.Effect<ProjectSetupScriptRunner.ProjectSetupScriptRunnerResult> => {
              ...
              if (setupResult === null) return { ready: false, pending: null };
              if (setupResult.status === "no-script" || !setupResult.completion) {
                return { ready: true, pending: null };
              }
              ...
              if (!setupResult.async) {
                return { ready: yield* Fiber.join(completionFiber), pending: null };
              }
              return { ready: true, pending: completionFiber };
            });
          ...
          const bootstrapProgram = Effect.gen(function* () {
            ...
            const pendingSetupScript =
              preparedWorkflowWorkspace === null ? (yield* runSetupProgram()).pending : null;
      ...
      const loadServerConfig = (options: { readonly usageLimitsCommand: boolean }) =>
        Effect.gen(function* () {
          ...
          return {
            ...
            settings,
            previewBrowser: yield* BrowserExecutableResolver.resolvePreviewBrowserStatus(config),
