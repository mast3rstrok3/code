...
import { FileIcon, BotIcon, CircleAlertIcon, PaperclipIcon, PlayIcon, XIcon } from "lucide-react";
...
export const ChatComposer = memo(function ChatComposer(props: ChatComposerProps) {
  ...
  useEffect(() => {
    if (requestedInteractionMode !== "default" || workflowPreset !== null) {
      setSelectedBuildSkillId(null);
    }
  }, [requestedInteractionMode, workflowPreset]);
  ...
  const { enabled: planModeUiEnabled, interactionMode: supportedInteractionMode } =
    resolveComposerInteractionMode({
      planModeEnabled: settings.planModeEnabled,
  ...
  const interactionMode =
    workflowPreset !== null ? requestedInteractionMode : supportedInteractionMode;
  ...
  const composerModeControls = useMemo<ComposerModeControls | null>(() => {
    if (!planModeUiEnabled && !planningWorkflowAvailable && buildSkills.length === 0) {
      return null;
    }
    return {
      interactionMode,
  ...
  const restingBlockDefs = [
      ...
      : []),
    {
      ...
      content: (
        <>
          {composerModeControls ? <ComposerModeControl controls={composerModeControls} /> : null}
