...
export default function ChatView(props: ChatViewProps) {
  ...
  // Reserved draft ids do not exist on the server until the first send, so
  // they must wait for the shell index just like the detail subscription.
  const routeThreadStateRef = resolveThreadDetailRef(
  ...
  const routeThreadState = useEnvironmentThread(
    routeThreadStateRef?.environmentId ?? null,
    routeThreadStateRef?.threadId ?? null,
  );
  ...
  useLayoutEffect(() => {
    ...
    if (
      ...
      activeThreadRef &&
      explicitThreadRef !== activeThreadRef
    ) {
      useDiffPanelStore.getState().selectGitScope(activeThreadRef, "unstaged");
    }
  }, [activeRightPanelKind, activeThreadRef]);
  ...
  const activeWorktreeAppReviewRun = useMemo(() => {
    ...
    return (
      appReviewWorkflowRuns.find((run) => {
        ...
        const target = activeWorkflowNavigationThreadShells.find(
          (thread) =>
            thread.environmentId === activeThread.environmentId && thread.id === run.targetThreadId,
        );
        const runTargetPath =
          target?.worktreePath ??
          (target?.projectId === activeProject?.id ? (activeProject?.workspaceRoot ?? null) : null);
  ...
  const productWorkflowPlanningWorkflow = useThreadPlanningWorkflow(
    isServerThread ? productWorkflowPlanningThreadRef : null,
  );
  ...
  const { enabled: interactionModeEnabled, interactionMode: supportedInteractionMode } =
    resolveComposerInteractionMode({
  ...
  const onInterrupt = useCallback(async () => {
    ...
    if (!input && cancelableRun === null) return;
    ...
    const result = await interruptThreadTurn({
      environmentId: activeThread.environmentId,
      input: input ?? buildThreadTurnInterruptInput(activeThread),
    });
    if (result._tag === "Failure" && !isAtomCommandInterrupted(result)) {
      const error = squashAtomCommandFailure(result);
      setThreadError(
  ...
  }, [handleComposerModeChange, interactionMode, interactionModeEnabled]);
  ...
  const onCompactContext = async () => {
    ...
    try {
      const settingsResult = await persistThreadSettingsForNextTurn({
        ...
        workflowImplementationSettings: activeThread?.workflowImplementationSettings ?? null,
      });
      const result =
  ...
  const onSend = async (
    ...
    if (failure === null && turnAttachmentsResult._tag === "Success") {
      ...
      const backgroundThreadRef =
        resolvedSubmissionIntent === "background" && !isAppReviewWorkflowSend
          ? scopeThreadRef(activeThread.environmentId, threadIdForSend)
          : null;
      if (backgroundThreadRef) {
        beginBackgroundDraftSubmissionByRef(backgroundThreadRef);
      }
      const startPromise = isAppReviewWorkflowSend
        ? launchAppReviewWorkflow({
            environmentId,
            input: {
  ...
  const rightPanelContent = activeThreadRef ? (
    renderedRightPanelSurface?.kind === "preview" ? (
    ...
    ) : renderedRightPanelSurface?.kind === "app-stack" && activeProject && activeWorkspaceRoot ? (
      <AppStackPanel
      ...
      />
    ) : renderedRightPanelSurface?.kind === "instructions" ? (
      <WorkflowInstructionsPanel
        environmentId={activeThread.environmentId}
        workflowPromptId={renderedRightPanelSurface.workflowPromptId}
      />
