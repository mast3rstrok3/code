            ...
            <div className="relative z-10 h-[4.875rem] px-[var(--sidebar-row-content-inset)] py-[var(--sidebar-content-inset)]">
              <div className="flex h-5 min-w-0 items-center gap-1.5">
                ...
                <ProjectFavicon
                  ...
                  className="size-4 shrink-0"
                />
                {workflowRoleLabel || props.projectDisplayName ? (
                  <span
                    className={cn(
  ...
  const {
    ...
    for (const thread of visible) {
      ...
      const lifecycle = resolveWorkflowLifecycle(members, (member) => {
        ...
        const snapshot = changeRequestSnapshotByKey.get(memberKey);
        const changeRequest =
          snapshot != null &&
          (member.linkedPullRequest == null
            ? member.worktreePath === null || snapshot.branch === member.branch
            : snapshot.linkedPullRequest?.projectId === member.linkedPullRequest.projectId &&
              snapshot.linkedPullRequest.repository === member.linkedPullRequest.repository &&
              snapshot.linkedPullRequest.number === member.linkedPullRequest.number)
            ? snapshot.pr
            : null;
