...
export function ThreadRouteView({ target }: { target: ThreadRouteTarget }) {
  ...
  const focusedWorkflowId = useSearch({
    strict: false,
    select: (search) => search.workflow ?? null,
  });
  ...
  // A client-reserved draft thread id does not exist on the server until the
  // first send, so its detail is not read until the shell lists it.
  const serverThreadDetailRef = resolveThreadDetailRef(serverThreadRef, {
    shellExists: serverThreadShell !== null,
    waitForShell: draftThread !== null,
  });
  const serverThreadDetail = useThreadDetail(serverThreadDetailRef);
  const serverThreadStatus = useThreadStatus(serverThreadDetailRef);
