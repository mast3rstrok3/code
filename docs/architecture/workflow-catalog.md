...
4. Requires and re-ensures the inherited AppDevStack, then probes it before Dev Review. Implementation never creates a replacement when the workflow-owned stack is missing.
5. Launches a nested Dev Review workflow with its own workflow ID and the Implementation workflow ID as its parent. AppDevStack repair remains Implementation-owned and does not consume Dev Review attempts. The nested workflow refreshes the frontend URL after every repair. Pass continues to Code Review and exhaustion continues with an unresolved-review warning. A blocked outcome makes Implementation re-ensure and probe the inherited stack and launch a fresh nested run, bounded to three consecutive automatic unblock attempts independently of the ten product-repair slots. The next blocked outcome requires human attention; an explicit retry resets that recovery window. Runs created before this strategy retain the legacy inline QA path.
